...
public class ImageLoader {
    ...
    private void initDiskCache() {
        try {
            File cacheDir = OtherUtils.getDiskCacheDir(GfeduApplication.getContext(), "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, OtherUtils.getAppVersion(GfeduApplication.getContext()),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    class DiskCacheThread extends Thread {
        ...
        public void run() {
            try {
                ...
                if (editor != null) {
//                    FileOutputStream outputStream = editor.newOutputStream(0);
    ...
    private boolean cacheBitmap2Disk(Bitmap bitmap, OutputStream outputStream) {
        Log.d(" ==** " ,""+outputStream);
        Log.d(" ==22 " ,""+DEFAULT_COMPRESS_FORMAT);
        Log.d(" ==33 " ,""+DEFAULT_COMPRESS_QUALITY);
        try {
