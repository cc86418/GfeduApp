...
public class AddNoteActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        subjectId = getIntent().getIntExtra("childClassType_LevelID", 0);
        // 请求类型
        afterType = getIntent().getIntExtra("afterType", 0);
        // 跳转标记(笔记OR答疑)
        jumpFlag = getIntent().getIntExtra("jumpFlag", 0);
        // 从视频或者做题传入情况
        videoOrTestFlag = getIntent().getIntExtra("videoOrTestFlag", 0);
        // 试卷解析传递试卷名称
                            ...
                            new Subscriber<CommonBean>() {
                                ...
                                public void onNext(CommonBean commonBean) {
                                    if (100 == commonBean.getCode()) {
                                        ...
                                        if (7 == videoOrTestFlag) { // 全部解析
                                            Intent changeIt = new Intent();
                                            changeIt.putExtra("count", 1);
                                            changeIt.setAction("com.jc.noteChange");
                                            baseActivity.sendBroadcast(changeIt);
                                        } else if (8 == videoOrTestFlag) { // 错题解析
                                            Intent wIt = new Intent();
                                            wIt.putExtra("count", 1);
                                            wIt.setAction("com.jc.wroNoteChange");
                                            baseActivity.sendBroadcast(wIt);
                                        } else { // 正常情况
    ...
    private void initView() {
        ...
        // 在线观看视频(传递了科目名称)
        if (6 == videoOrTestFlag) {
            // 设置科目名称
            subject_txt.setText(ToolUtils.strReplaceAll(videoName));
            select_item_layout.setEnabled(false);
            select_item_layout.setClickable(false);
            comingImg.setVisibility(View.GONE);
        } else if (8 == videoOrTestFlag || 7 == videoOrTestFlag) {
        ...
        // 创建适配器
        adapter = new ImageRecyclerAdapter(this, onPicClickListener, mHandler);
        // 添加数据
        adapter.setList(itemList);
        if (0 == jumpFlag) {
            // 添加默认个数
            adapter.setSelectMax(1);
        } else if (1 == jumpFlag || 2 == jumpFlag) {
            // 添加默认个数
                            ...
                            new Subscriber<CommonBean<Question>>() {
                                ...
                                public void onNext(CommonBean<Question> quesInfoCommonBean) {
                                    if (100 == quesInfoCommonBean.getCode()) {
                                        ...
                                        if (7 == videoOrTestFlag) { // 全部解析情况
                                            Intent changeIt = new Intent();
                                            changeIt.putExtra("count", 1);
                                            changeIt.setAction("com.jc.quesChange");
                                            baseActivity.sendBroadcast(changeIt);
                                        } else if (8 == videoOrTestFlag) { // 错题解析情况
                                            Intent wIt = new Intent();
                                            wIt.putExtra("count", 1);
                                            wIt.setAction("com.jc.wroQuesChange");
                                            baseActivity.sendBroadcast(wIt);
                                        } else {
                                            Intent it = new Intent();
    ...
    private RequestBody commRequestBody(String quesContent) {
        ...
        try {
            ...
            if (4 == afterType) {
                ...
                // 课程id
                obj.put("classScheduleId", lessonId);
                // 试卷id
                obj.put("testPaperId", testPapgerId);
                // 试题id
