...
public class QuestionDetailActivity extends BaseActivity {
    ...
    private BroadcastReceiver receiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if ("com.jc.commentCount".equals(intent.getAction())) { // 评论数量
                // 评论数量
                Bundle bundle = intent.getExtras();
                int answerId = bundle.getInt("answerId");
                int count = bundle.getInt("count");
                for (int i = 0; i < answerList.size(); i++) {
                    if (answerId == answerList.get(i).getAnswerId()) {
                        Answer answer = new Answer();
                        answer.setUserPraiseStatus(answerList.get(i).getUserPraiseStatus());
                        answer.setImageCount(answerList.get(i).getImageCount());
                        int oriCount = answerList.get(i).getCommentsCount();
                        answer.setCommentsCount(oriCount + count);
                        answer.setAfterQuesCount(answerList.get(i).getAfterQuesCount());
                        answer.setAnswerAddTime(answerList.get(i).getAnswerAddTime());
                        answer.setAnswerContent(answerList.get(i).getAnswerContent());
                        answer.setAnswerId(answerList.get(i).getAnswerId());
                        answer.setAnswerImageUrl(answerList.get(i).getAnswerImageUrl());
                        // 点赞数量加1
                        answer.setPraiseCount(answerList.get(i).getPraiseCount());
                        answer.setAnswerStatus(answerList.get(i).getAnswerStatus());
                ...
                // 重新请求刷新数据
                setRefresh();
            } else if ("com.jc.changeQues".equals(intent.getAction())) {
                // 重新请求刷新数据
                setRefresh();
            }
        }
                            ...
                            public void onNext(CommonBean<QuesDetailBean> quesDetailBeanCommonBean) {
                                if (100 == quesDetailBeanCommonBean.getCode()) {
                                    ...
                                    if (null != quesContentBean) {
                                        if (3 == quesContentBean.getQuesStatus()) {
                                        ...
                                        // 问题内容
                                        quesDetail_txt.setText(ToolUtils.strReplaceAll(quesContentBean.getQuesContent()));
                                        // 获取图片列表
                                        ...
                                        // 记录人
                                        recordName_txt.setText("提问人：" + ToolUtils.strReplaceAll(quesContentBean.getNickName()));
                                        // 获取关联视频
                                        ...
                                        if (null != answerList && answerList.size() > 0) {
                                            ...
                                            adapter = new AnswerRecyclerAdapter(baseActivity, answerList, jumpFlag, quesId, quesContentBean.getQuesStatus());
    ...
    void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.register_txt: // 删除or我要作答
                ...
                break;
            case R.id.relation_layout: // 相关联视频
                String vid = quesContentBean.getVID();
                String videoId = String.valueOf(quesContentBean.getVideoId());
                String StudyKey = quesContentBean.getVideo_StudyKey();
                String title = quesContentBean.getVideoName();
                String strClassId = String.valueOf(classId);
                String strStageId = String.valueOf(childClassTypeId);
                ...
                if (null != vid && !"".equals(vid)) {
                    IjkVideoActicity.intentTo(baseActivity, IjkVideoActicity.PlayMode.landScape, IjkVideoActicity.PlayType.vid, vid,
                            true, title, strClassId, strStageId, videoId, StudyKey, vid, "0", stageProblemStatus, stageNoteStatus, stageInformationStatus);
                }
                break;
...
//                );
    ...
    private RequestBody commRequest(int page) {
        ...
        try {
            ...
            obj.put("stageId", childClassTypeId);
