...
public class RegisterActivity extends BaseActivity {
    ...
    private void sendPhoneMessage(String phoneStr) {
        ...
        Observable<CommonBean> observable = httpPostService.getSendMsgInfo(body);
                ...
                .subscribe(
                        new Subscriber<CommonBean>() {
                            ...
                            public void onNext(CommonBean commonBean) {
                                if (100 == commonBean.getCode()) { // 成功情况
                                    // 发送消息，并启动倒计时
                                    timeCount.start();
                                    clickFlag = false;
                                } else { // 发送错误情况
                                    setErrorInfo(errorVerity_tap_txt, commonBean.getMessage());
                                }
                            }
                        }
                );
     ...
     * @param pwd       密码
     */
    private void getRegeistgerInfoFromHttp(final String nikeName, final String phoneName, final String pwd) {
        ...
        try {
            ...
            obj.put("RealName", nikeName);
            obj.put("Telephone", phoneName);
        ...
        RequestBody body = RequestBody.create(MediaType.parse("application/json"), obj.toString());
        // 发送请求并返回请求数据
        Observable<CommonBean<Register>> observable = httpPostService.getRegisterInfo(body);
        observable.subscribeOn(Schedulers.io())
                .unsubscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(
                        new Subscriber<CommonBean<Register>>() {
                            @Override
                            public void onCompleted() {
                            ...
                            @Override
                            public void onNext(CommonBean<Register> registerCommonBean) {
                                if (100 == registerCommonBean.getCode()) {
                                    Toast.makeText(RegisterActivity.this, "注册成功,2秒后自动跳转登录页", Toast.LENGTH_SHORT).show();
                                    new Handler().postDelayed(new Runnable() {
                                        @Override
                                        public void run() {
                                            Intent it = new Intent();
                                            it.setAction("com.jc.finishAc");
                                            baseActivity.sendBroadcast(it);
                                            baseActivity.openActivityAndCloseThis(NormalActivity.class);
                                        }
                                    }, 2000);
                                } else {
                                    Toast.makeText(RegisterActivity.this, registerCommonBean.getMessage(), Toast.LENGTH_SHORT).show();
                                }
                            }
