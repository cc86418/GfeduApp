                    ...
                    .subscribe(new Subscriber<CommonBean<StageInfo>>() {
                        ...
                        public void onNext(CommonBean<StageInfo> stageInfoCommonBean) {
                            if (100 == stageInfoCommonBean.getCode()) {
                                ...
                                String str =stageInfoCommonBean.getBody().getCountWord();
                                timeDru_Txt.setText(str);
                                String couDay = cutDownDayStr + "天";
                                day_Txt.setText(ToolUtils.setTextSize(baseActivity, couDay, couDay.length() - 1, couDay.length(), R.style.style0, R.style.style1), TextView.BufferType.SPANNABLE);
                                if (null != mData && mData.size() > 0) {
                                    if (empty_layout.getVisibility() == View.VISIBLE) {
                                        empty_layout.setVisibility(View.GONE);
                                    }
                                    // 初始化卡片适配器
                                    cardPagerAdapter = new CardPagerAdapter(baseActivity, classId, strTitle);
                                    // 添加数据
