...
public class QuestionAllActivity extends BaseActivity {
    ...
    private void setContent() {
        ...
        // 班级id
        bundle.putInt("classTypeId", classId);
        // 阶段id
        bundle.putInt("childClassTypeId", stageId);
        // 试卷id
        bundle.putInt("testPaperId", testPaperId);
        // 试题id
        bundle.putInt("testQuesId", testQuesId);
        // 视频id
        bundle.putInt("videoId", videoId);
        // 课程id
        bundle.putInt("classScheduleId", lessonId);
    ...
    class MyFragmentAdapter extends FragmentPagerAdapter {
        ...
        public void setPrimaryItem(ViewGroup container, int position, Object object) {
            ...
            switch (position) {
                case 0: // 我的问题
                    floatAction_Btn.setVisibility(View.VISIBLE);
                    break;
                case 1: // 大家的问题
                    floatAction_Btn.setVisibility(View.GONE);
                    break;
                case 2: // 我的回答
                    floatAction_Btn.setVisibility(View.GONE);
                    break;
    ...
    void OnClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.noteMore_Btn: // 搜索按钮
                ...
                // 班级id
                searchBundle.putInt("classTypeId", classId);
                // 阶段id
                searchBundle.putInt("childClassTypeId", stageId);
                // 试卷id
                searchBundle.putInt("testPaperId", testPaperId);
                // 试题id
                searchBundle.putInt("testQuesId", testQuesId);
                // 视频id
                searchBundle.putInt("videoId", videoId);
                // 课表id
                searchBundle.putInt("classScheduleId", lessonId);
                int status = viewPager.getCurrentItem();
                if (0 == status) { // 我的答疑
                    searchBundle.putInt("type", 1);
                    baseActivity.openActivity(QuestionSearchAc.class, searchBundle);
                } else if (1 == status) { // 大家的答疑
                    searchBundle.putInt("type", 2);
                    baseActivity.openActivity(QuestionSearchAc.class, searchBundle);
                } else if (2 == status) { // 我的回答
                    baseActivity.openActivity(MyAnswerSearchAc.class, searchBundle);
                }
            ...
            case R.id.floatAction_Btn: // 悬浮按钮
                ...
                bundle.putInt("childClassTypeId", stageId);
                bundle.putInt("testQuesId", testQuesId);
                bundle.putInt("testPapgerId", testPaperId);
                bundle.putInt("videoId", videoId);
                bundle.putInt("videoOrTestFlag", videoOrTestFlag);
