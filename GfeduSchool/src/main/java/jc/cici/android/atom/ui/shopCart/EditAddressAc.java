...
/**
 * 编辑收货地址Activity
 * Created by atom on 2017/8/21.
 */

public class EditAddressAc extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // 地址id
        addressPKID = bundle.getInt("addressPKID", 0);
        // 收货人名称
        addressName = bundle.getString("addressName");
        // 收货人联系方式
        addressMobile = bundle.getString("addressMobile");
        // 收货人所在省份
        addressProvince = bundle.getString("addressProvince");
        // 收货人所在市份
        addressCity = bundle.getString("addressCity");
        // 详细地址
    ...
    private void initView() {
        ...
        // 收货人名称
        receiverName_txt.setText(addressName);
        // 手机号
        contactName_txt.setText(addressMobile);
        SNProvince = addressProvince;
        SNCountry = addressCity;
        // 所在区域
        areaName_txt.setText(addressProvince + "-" + addressCity);
        // 详情
        detailAddressName_txt.setText(addressDetail);
    }


    @OnClick({R.id.back_layout, R.id.area_layout, R.id.del_txt, R.id.saveBtn})
    void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.area_layout: // 所在地区布局
                ...
                break;
            case R.id.del_txt: // 删除文字
                deleteAddress(addressPKID);
                break;
                            ...
                            if (null != strareaName && !"".equals(strareaName)) {
                                ...
                                if (null != strdetailAddressName && !"".equals(strdetailAddressName)) {
                                    ...
                                    if (NetUtil.isMobileConnected(baseActivity)) {
                                        // 提交收货地址信息
                                        submitAddressInfo(strreceiverName, strcontactName, strareaName, strdetailAddressName);
                                    } else {
     ...
     */
    private void submitAddressInfo(String strreceiverName, String strcontactName, String strareaName, String strdetailAddressName) {
        ...
        Observable<CommonBean> observable = httpPostService.updateAddressInfo(body);
        observable.subscribeOn(Schedulers.io())
                ...
                .subscribe(new Subscriber<CommonBean>() {
                    ...
                    public void onNext(CommonBean commonBean) {
                        if (100 == commonBean.getCode()) {  // 地址修改成功
                            Toast.makeText(baseActivity, "修改成功", Toast.LENGTH_SHORT).show();
                            Intent it = new Intent();
                            it.setAction("com.update.address");
     ...
     * @param addressPKID
     */
    private void deleteAddress(int addressPKID) {
        Retrofit retrofit = RetrofitOKManager.getinstance().doBaseRetrofit(Global.BASE_URL);
        HttpPostService httpPostService = retrofit.create(HttpPostService.class);
        JSONObject obj = new JSONObject();
        CommParam commParam = new CommParam(baseActivity);
        ...
        observable.subscribeOn(Schedulers.io())
                ...
                .subscribe(new Subscriber<CommonBean>() {
                    ...
                    @Override
                    public void onError(Throwable e) {
                        Toast.makeText(baseActivity, "网络请求异常,删除失败", Toast.LENGTH_SHORT).show();
                    }
