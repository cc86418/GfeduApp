...
//                Bundle bundle = intent.getExtras();
                System.out.println("com.jc.commentCount" + ",count >>>" + intent.getIntExtra("count", 0));
                // 评论数量
                int count = intent.getIntExtra("count", 0);
                int allCount = zhuiwenCount + count;
                commitCount_txt.setText("(" + allCount + ")");
            }
        }
    };
                        ...
                        new Subscriber<CommonBean<ZhuiWenBean>>() {
                            ...
                            public void onNext(CommonBean<ZhuiWenBean> zhuiWenBeanCommonBean) {
                                if (100 == zhuiWenBeanCommonBean.getCode()) {
                                    ...
                                    if (1 != zhuiWenBeanCommonBean.getBody().getAnswerInfo().getAnswerStatus()) {
                                    ...
                                    // 设置点赞数量
                                    zhuiwenCount = zhuiWenBeanCommonBean.getBody().getAnswerInfo().getCommentsCount();
                                    commitCount_txt.setText("(" + zhuiwenCount + ")");
                                    // 设置评论数量
                                    ...
                                    if (null == afterList) { // 没有追问情况(只显示问题和答案)
                                        ...
                                        // 添加公共内容
                                        setCommonContent(zhuiWenBeanCommonBean.getBody().getQuesInfo(), zhuiWenBeanCommonBean.getBody().getAnswerInfo());
                                        for (int i = 0; i < afterList.size(); i++) { // 追问追答情况
                                            if ("".equals(afterList.get(i).getImageUrl())) { // 无图片情况
                                                ZhuiWen zhuiWen = new ZhuiWen();
                                                zhuiWen.setId(afterList.get(i).getId());
                                                zhuiWen.setContent(afterList.get(i).getContent());
                                                zhuiWen.setImageUrl("");
                                                zhuiWen.setStudentID(afterList.get(i).getStudentID());
                                                System.out.println("afterList.get(i).getNickName() >>>:"+afterList.get(i).getNickName());
                                                zhuiWen.setNickName(afterList.get(i).getNickName());
                                                zhuiWen.setToNickName(afterList.get(i).getToNickName());
                                                zhuiWen.setAddTime(afterList.get(i).getAddTime());
                                                zhuiWen.setRoleType(afterList.get(i).getRoleType());
                                                mList.add(zhuiWen);
                                            } else if (afterList.get(i).getImageUrl().length() > 0) { // 有图片情况
                                                ...
                                                zhuiWen2.setRoleType(afterList.get(i).getRoleType());
                                                mList.add(zhuiWen2);
                                            }
                                        }
                                    }
    ...
    private void setCommonContent(Question question, Answer answer) {
        ...
        if (null != question.getQuesImageUrl() && question.getQuesImageUrl().size() > 0) { // 有图片情况
            if (question.getQuesImageUrl().size() == 1) {
                ...
                ZhuiWen zhuiWen0 = new ZhuiWen();
                zhuiWen0.setId(0);
                zhuiWen0.setContent(question.getQuesContent());
                zhuiWen0.setImageUrl("");
                if (1 == canZhuiDa && 0 == canZhuiWen) { // 追答第一条居左
                    zhuiWen0.setStudentID(0);
                } else if (1 == canZhuiWen && 0 == canZhuiDa) { //追问 第一条居右
                    zhuiWen0.setStudentID(ToolUtils.getUserID(baseActivity));
                }
                zhuiWen0.setNickName(question.getNickName());
                zhuiWen0.setToNickName("");
                zhuiWen0.setHeadImg(question.getHeadImg());
                zhuiWen0.setAddTime(question.getQuesAddTime());
                zhuiWen0.setRoleType(question.getRoleType());
                mList.add(zhuiWen0);
                ...
                zhuiWen1.setRoleType(question.getRoleType());
                mList.add(zhuiWen1);
        ...
        if (null != answer.getAnswerImageUrl() && answer.getAnswerImageUrl().size() > 0) { // 有图片情况
            ...
            if (answer.getAnswerImageUrl().size() == 1) {
                ZhuiWen zhuiWen0 = new ZhuiWen();
                zhuiWen0.setId(0);
                zhuiWen0.setContent(answer.getAnswerContent());
                zhuiWen0.setImageUrl("");
                ...
                zhuiWen0.setNickName(answer.getNickName());
                zhuiWen0.setToNickName("");
                zhuiWen0.setHeadImg(answer.getHeadImg());
                zhuiWen0.setAddTime(answer.getAnswerAddTime());
                zhuiWen0.setRoleType(answer.getRoleType());
                mList.add(zhuiWen0);
    ...
    void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.zhuiwenCount_txt: // 追问(追答)监听
                String str = zhuiwenCount_txt.getText().toString();
                // 获取追问，追答文字
                String titleName = str.substring(2,str.length());
