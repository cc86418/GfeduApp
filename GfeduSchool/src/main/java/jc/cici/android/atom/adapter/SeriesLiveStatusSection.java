...
public class SeriesLiveStatusSection extends StatusSection {

    private Context mCtx;
    private ArrayList<LiveContent.SerContent> mSeriesLiveList;
    // 当前项目id
    private int mProjectId;
    // 当前项目名称
    private String mTitle;
    // 判断当前系列直播是否为第一条(1:第一条,0:非第一条)
    private int mFlag;

    public SeriesLiveStatusSection(Context context, ArrayList<LiveContent.SerContent> seriesLive, int projectId, String title, int flag) {
        super(R.layout.item_title_live, R.layout.item_section_content_live);
        this.mCtx = context;
        this.mSeriesLiveList = seriesLive;
    ...
    public int getContentItemsTotal() {
        return null != mSeriesLiveList ? mSeriesLiveList.size() : 0;
    }
    ...
    public void onBindItemViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ...
        LiveContent.SerContent seriesContent = mSeriesLiveList.get(position);
        // 直播课程背景图片
        Glide.with(mCtx).load(seriesContent.getClass_MobileImage())
        ...
        //  判断是否含有公开课
        String salePrice = seriesContent.getClass_PriceSaleRegion();
        ...
        // 隐藏系列直播时间
        itemViewHolder.time_layout.setVisibility(View.GONE);

        // 直播课程时间
        itemViewHolder.courseTime_txt.setText(
                seriesContent.getCS_DateShort() + " "
                        + seriesContent.getCS_StartTime() + "-"
                        + seriesContent.getCS_EndTime());
        // 直播课程标题
        itemViewHolder.courseName_txt.setText(ToolUtils.strReplaceAll(seriesContent.getClass_Name()));
        // 图片监听
        itemViewHolder.product_course_img.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // 获取班级id
                int classPkID = mSeriesLiveList.get(position).getClass_PKID();
                // 获取系列直播or普通直播
                int classFrom = mSeriesLiveList.get(position).getClass_Form();
                switch (classFrom) {
                    case 1: // 系列直播
...
//                        norBundle.putInt("Product_PKID", classPkID);
    ...
    public void onBindHeaderViewHolder(RecyclerView.ViewHolder holder) {
        ...
        headerHolder.lineImg.setBackgroundResource(R.drawable.icon_series);
        headerHolder.headerName_txt.setText("系列直播");
        if (1 == mFlag) { // 系列直播在第一条情况
            headerHolder.moreLayout.setVisibility(View.VISIBLE);
        } else { // 非第一条情况
            headerHolder.moreLayout.setVisibility(View.GONE);
        }
