...
public class ChildDoubleRecAdapter extends RecyclerView.Adapter<ChildDoubleRecAdapter.MyHolder> {

    private Context mCtx;
    private ArrayList<PackageProduct> mItems;
    // 订单来源状态
    ...
    public ChildDoubleRecAdapter(Context context, ArrayList<PackageProduct> items, int orderStatus) {
        this.mCtx = context;
        this.mItems = items;
...
//            }
        ...
        switch (classType) {
            case 1: // 面授
                ...
                holder.courseTime_txt.setText("课程有效期：" + mItems.get(position).getClassType().getExpireDate());
                break;
            case 2: // 在线
                ...
                if (1 == mItems.get(position).getClassType().getClassType_Mode()) { // 固定时间
                    holder.courseTime_txt.setText("课程有效期：" + ToolUtils.strReplaceAll(mItems.get(position).getClassType().getExpireDate()));
                } else if (2 == mItems.get(position).getClassType().getClassType_Mode()) { // 售后即开
                    holder.courseTime_txt.setText("课程有效期：" + mItems.get(position).getClassType().getClassType_StudyDay());
                }
                break;
            case 4: // 直播
                holder.classAddress_layout.setVisibility(View.GONE);
