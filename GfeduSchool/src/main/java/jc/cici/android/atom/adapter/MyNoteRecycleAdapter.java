...
public class MyNoteRecycleAdapter extends BaseRecycleerAdapter<Note, MyNoteRecycleAdapter.MyHolder> {
    ...
    public MyNoteRecycleAdapter(Context context, ArrayList<Note> items, int status) {
        super(context, items);
        this.mCtx = context;
        this.mList = items;
        this.status = status;
    ...
    @Override
    public void onBindViewHolder(MyHolder holder, Note item, final int position) {
        ...
        if (2 == status) { // 大家笔记
            ...
            // 用户名
            holder.userName.setText(ToolUtils.strReplaceAll(item.getS_NickName()));
