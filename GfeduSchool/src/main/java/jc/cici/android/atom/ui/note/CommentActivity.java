...
public class CommentActivity extends BaseActivity {
    ...
    private void setContent() {
        ...
        RequestBody body = commRequest(1);
        Observable<CommonBean<CommentBean>> observable = httpPostService.getCommentListInfo(body);
        observable.subscribeOn(Schedulers.io())
                .unsubscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(
                        new Subscriber<CommonBean<CommentBean>>() {
                            @Override
                            ...
                            @Override
                            public void onNext(CommonBean<CommentBean> zhuiWenBeanCommonBean) {
...
//        linearLayoutManager.setReverseLayout(true);
     ...
     * @return
     */
    private RequestBody commRequest(int flag) {
        if (1 == flag) {
            // 加载数据
    ...
    private RequestBody commRequestBody(String content) {
        ...
        try {
            ...
            //问题id
            obj.put("answerId", answerId);
            if (null != commentList && !"null".equals(commentList) && commentList.size() > 0) {
                //回复id
                obj.put("replyId", commentList.get(mPos).getId());
            } else {
                //回复id
                obj.put("replyId", 0);
            }
    ...
    private void requestAgain() {
        Retrofit retrofit = RetrofitOKManager.getinstance().doBaseRetrofit(Global.BASE_URL);
        HttpPostService httpPostService = retrofit.create(HttpPostService.class);
        RequestBody body = commRequest(0);
        Observable<CommonBean<CommentBean>> observable = httpPostService.getCommentListInfo(body);
        observable.subscribeOn(Schedulers.io())
                .unsubscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                        ...
                        new Subscriber<CommonBean<CommentBean>>() {
                            ...
                            public void onNext(CommonBean<CommentBean> zhuiWenBeanCommonBean) {
                                if (100 == zhuiWenBeanCommonBean.getCode()) {
                                    ...
                                    it.putExtra("count", 1);
                                    it.setAction("com.jc.commentCount");
