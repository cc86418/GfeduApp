...
public class HistoryRecyclerAdapter extends BaseRecycleerAdapter<HistoryLesson, HistoryRecyclerAdapter.MyHolder> {
    ...
    public void onBindViewHolder(MyHolder holder, HistoryLesson item, int position) {
        ...
        String[] dayStr = str[2].split(" ");
        String subStr = dayStr[0];
        String newStr = subStr + "日" + str[1] + "月";
        holder.date_Txt.setText(ToolUtils.setTextSize(mCtx, newStr, newStr.length() - 3, newStr.length(), R.style.style4, R.style.style5));
        ...
        if (0 == status) { // 缺勤状态
            ...
            holder.attendenceFlag_img.setVisibility(View.VISIBLE);
        } else if (1 == status) { // 出勤状态
            holder.attendenceFlag_img.setBackgroundResource(R.drawable.icon_chuqin);
            holder.attendenceFlag_img.setVisibility(View.VISIBLE);
        } else if (2 == status) { // 未登记
            holder.attendenceFlag_img.setVisibility(View.GONE);
        }
    }
