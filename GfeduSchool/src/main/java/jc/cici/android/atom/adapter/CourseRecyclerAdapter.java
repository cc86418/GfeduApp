    ...
    public void onBindViewHolder(MyHolder holder, CourseInfo item, int position) {
        ...
        if (holder instanceof MyHolder) {
            ...
            Glide.with(mCtx).load(item.getTeacherImg())
                    ...
                    .error(R.drawable.teacher_img) //加载失败时显示的图片
                    .crossFade(1000) //淡入显示的时间,注意:如果设置了这个,则必须要去掉asBitmap
                    .override(90, 90) // 设置最终显示图片大小
                    .centerCrop() // 中心剪裁
            ...
            // 老师名称
            myHolder.teacherName_Txt.setText(ToolUtils.strReplaceAll(item.getTeacherName()));
            // 课程名称
            ...
            switch (item.getLessonType()) {
                case 1: // 面授
                    ...
                    holder.coming_Img.setVisibility(View.VISIBLE);
                    break;
                case 2: // 直播
                    // 课程地点
            ...
            String newStr = str.replace("日", "");
            if(position != 0){
                myHolder.today_Txt.setText(ToolUtils.setTextSize(mCtx, newStr, newStr.length() - 4, newStr.length(), R.style.style2, R.style.style3), TextView.BufferType.SPANNABLE);
            }

            /**
             * 如果是直播/测试情况
             */
            myHolder.address_Txt.setOnClickListener(new View.OnClickListener() {
                @Override
