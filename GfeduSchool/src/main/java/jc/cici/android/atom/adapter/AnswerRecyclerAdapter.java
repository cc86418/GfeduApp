...
public class AnswerRecyclerAdapter extends BaseRecycleerAdapter<Answer, AnswerRecyclerAdapter.MyHolder> {
    ...
    public AnswerRecyclerAdapter(Context context, ArrayList<Answer> items, int status, int quesId,int quesStatus) {
        super(context, items);
        this.mCtx = context;
        this.mItems = items;
        this.mStatus = status;
        this.mQuesId = quesId;
        this.mQuesStatus = quesStatus;
    }
    ...
    public void onBindViewHolder(final MyHolder holder, Answer item, final int position) {
        ...
        // 追问追答数设置
        holder.answerCount_txt.setText("(" + item.getAfterCount() + ")追问追答");
         ...
         * 评论按钮监听
         */
        holder.commit_Btn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
