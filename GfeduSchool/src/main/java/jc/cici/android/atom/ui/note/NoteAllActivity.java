    ...
    void OnClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.noteMore_Btn: // 搜索按钮
                ...
                noteBundle.putInt("cspkid", cspkid);
                noteBundle.putInt("stageId", stageId);
                if (0 == status) { // 我的笔记
                    noteBundle.putInt("type", 1);
                } else if (1 == status) { // 大家的笔记
                    noteBundle.putInt("type", 2);
                }
                noteBundle.putInt("testQuesId", testQuesId);
                noteBundle.putInt("videoId", videoId);
                baseActivity.openActivity(NoteSearchActivity.class, noteBundle);
                break;
